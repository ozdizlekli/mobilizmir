...
# Find Anasayfa and set it
FRONT_ID=$(docker compose exec -T wordpress wp post list --post_type=page --name=anasayfa --field=ID --allow-root | head -n1)
if [ ! -z "$FRONT_ID" ]; then
  docker compose exec -T wordpress wp option update page_on_front $FRONT_ID --allow-root
fi
# Find Blog page and set it
BLOG_ID=$(docker compose exec -T wordpress wp post list --post_type=page --name=blog --field=ID --allow-root | head -n1)
if [ ! -z "$BLOG_ID" ]; then
  docker compose exec -T wordpress wp option update page_for_posts $BLOG_ID --allow-root
fi

# Clean up default "Hello World" post if exists
docker compose exec -T wordpress wp post delete 1 --force --allow-root || true
...
HTML
docker compose exec -T wordpress wp post create ./post1.html --post_title="Seramik Kaplama mı, Pasta Cila mı? Hangisini Seçmelisiniz?" --post_status=publish --post_category=blog --allow-root
